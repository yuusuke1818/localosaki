...
/**
 * The persistent class for the t_dm_day_rep_point database table.
 *
 */
@Entity
@Table(name="t_dm_day_rep_point")
@NamedQuery(name="TDmDayRepPoint.findAll", query="SELECT t FROM TDmDayRepPoint t")
public class TDmDayRepPoint implements Serializable {
    private static final long serialVersionUID = 1L;

    @EmbeddedId
    private TDmDayRepPointPK id;
    ...
    @ManyToOne
    @JoinColumns({
        @JoinColumn(name="point_no", nullable=false, insertable=false, updatable=false),
        @JoinColumn(name="sm_id", nullable=false, insertable=false, updatable=false)
        })
    private MSmPoint MSmPoint;

    public TDmDayRepPoint() {
    }

    public TDmDayRepPointPK getId() {
        return this.id;
    }

    public void setId(TDmDayRepPointPK id) {
        this.id = id;
    }
