...
import jp.co.osaki.osol.api.result.smcontrol.extract.FVxManageInfoSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.TSmStub;

public class FVxManageInfoSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static TSmStub stub;

	private static String API_ID = SmConstants.A200014_TEST;

	private static int SEND_PORT = 3002;
	...
	public static void setUpBeforeClass() throws Exception {
		...
		if (SmConstants.IS_STUB) {
			...
			// 機能用のスタブ準備
			stub = new TSmStub(API_ID, SEND_PORT);
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<FVxManageInfoSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<FVxManageInfoSelectExtractResult>>(){}.getType());
		FVxManageInfoSelectExtractResult paramResult = param.getResult();
		...
		FVxManageInfoSelectExtractResult expected = null;
		...
		try {
			...
			System.out.println("outRb.getString(SmConstants.JSON_RESULT)"+outRb.getString(SmConstants.JSON_RESULT));
			// 期待値生成
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<FVxManageInfoSelectExtractResult>(){}.getType());
