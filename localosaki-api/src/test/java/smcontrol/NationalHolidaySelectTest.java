...
import jp.co.osaki.osol.api.result.smcontrol.extract.NationalHolidaySelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class NationalHolidaySelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200051_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<NationalHolidaySelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<NationalHolidaySelectExtractResult>>(){}.getType());
		NationalHolidaySelectExtractResult paramResult = param.getResult();
		...
		NationalHolidaySelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<NationalHolidaySelectExtractResult>(){}.getType());
