...
import jp.co.osaki.osol.api.result.smcontrol.extract.SchedulePatternSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class SchedulePatternSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200061_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<SchedulePatternSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<SchedulePatternSelectExtractResult>>(){}.getType());
		SchedulePatternSelectExtractResult paramResult = param.getResult();
		...
		SchedulePatternSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<SchedulePatternSelectExtractResult>(){}.getType());
