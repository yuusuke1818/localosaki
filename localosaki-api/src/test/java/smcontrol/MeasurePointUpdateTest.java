...
import jp.co.osaki.osol.api.result.smcontrol.extract.MeasurePointSelectExtractResult;
import jp.co.osaki.osol.mng.constants.SmControlConstants;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class MeasurePointUpdateTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200056_TEST;
	...
	public void test_001() {
		...
		try {
			...
			String strMeasurePointInfo = inRb.getString(SmConstants.PARAM_MEASURE_POINT_INFO);
			...
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					...
					SmConstants.PARAM_MEASURE_POINT_INFO + "=" + strMeasurePointInfo + "&" +
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<MeasurePointSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<MeasurePointSelectExtractResult>>(){}.getType());
		MeasurePointSelectExtractResult paramResult = param.getResult();
		...
		MeasurePointSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<MeasurePointSelectExtractResult>(){}.getType());
