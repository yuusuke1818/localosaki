...
import jp.co.osaki.osol.api.result.smcontrol.extract.UnitSelectEaExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class UnitSelectEaTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200053_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<UnitSelectEaExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<UnitSelectEaExtractResult>>(){}.getType());
		UnitSelectEaExtractResult paramResult = param.getResult();
		...
		UnitSelectEaExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<UnitSelectEaExtractResult>(){}.getType());
