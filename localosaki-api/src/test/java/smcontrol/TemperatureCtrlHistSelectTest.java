...
import jp.co.osaki.osol.api.result.smcontrol.extract.TemperatureCtrlHistSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class TemperatureCtrlHistSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200028_TEST;
...
//
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<TemperatureCtrlHistSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<TemperatureCtrlHistSelectExtractResult>>(){}.getType());
		TemperatureCtrlHistSelectExtractResult paramResult = param.getResult();
		...
		TemperatureCtrlHistSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<TemperatureCtrlHistSelectExtractResult>(){}.getType());
