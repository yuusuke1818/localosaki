...
public class OutputTerminalCtrlSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200010_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<OutputTerminalCtrlSelectResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<OutputTerminalCtrlSelectResult>>(){}.getType());
		OutputTerminalCtrlSelectResult paramResult = param.getResult();
		...
		OutputTerminalCtrlSelectResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<OutputTerminalCtrlSelectResult>(){}.getType());
