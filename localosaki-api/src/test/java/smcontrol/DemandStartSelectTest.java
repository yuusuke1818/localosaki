...
public class DemandStartSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200024_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<DemandStartSelectResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<DemandStartSelectResult>>(){}.getType());
		DemandStartSelectResult paramResult = param.getResult();
		...
		DemandStartSelectResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<DemandStartSelectResult>(){}.getType());
