...
import jp.co.osaki.osol.api.result.smcontrol.extract.AnswerBackSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class AnswerBackSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200022_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<AnswerBackSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<AnswerBackSelectExtractResult>>(){}.getType());
		AnswerBackSelectExtractResult paramResult = param.getResult();
		...
		AnswerBackSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<AnswerBackSelectExtractResult>(){}.getType());
