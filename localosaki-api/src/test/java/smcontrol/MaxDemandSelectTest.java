...
public class MaxDemandSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200040_TEST;
	...
	public void test_001() {
		...
		try {
			...
			String strDataHist = inRb.getString(SmConstants.PARAM_DATA_HIST);

			// リクエスト生成
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					SmConstants.PARAM_SMID + "=" + strSmId + "&" +
					SmConstants.PARAM_DATA_HIST + "=" + strDataHist;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<MaxDemandSelectResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<MaxDemandSelectResult>>(){}.getType());
		MaxDemandSelectResult paramResult = param.getResult();
		...
		MaxDemandSelectResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<MaxDemandSelectResult>(){}.getType());
