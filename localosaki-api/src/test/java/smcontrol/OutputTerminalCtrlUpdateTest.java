...
public class OutputTerminalCtrlUpdateTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200011_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<OutputTerminalCtrlUpdateResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<OutputTerminalCtrlUpdateResult>>(){}.getType());
		OutputTerminalCtrlUpdateResult paramResult = param.getResult();
		...
		OutputTerminalCtrlUpdateResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<OutputTerminalCtrlUpdateResult>(){}.getType());
