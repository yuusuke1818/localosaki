...
import jp.co.osaki.osol.api.result.smcontrol.extract.SettingEventCtrlSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class AielMasterAreaConfigUpdateTest {

    private static SmControlTestUtil util = new SmControlTestUtil();

    private static SmStub stub;

    private static String API_ID = SmConstants.A210006_TEST;
    ...
    private void commonExecute(String strParam){
        ...
        // レスポンスからGsonパースで値セット
        SmControlApiResponse<SettingEventCtrlSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<SettingEventCtrlSelectExtractResult>>(){}.getType());
        SettingEventCtrlSelectExtractResult paramResult = param.getResult();
        ...
        SettingEventCtrlSelectExtractResult expected = null;
        ...
        try {
            ...
            if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
                expected =
                        new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<SettingEventCtrlSelectExtractResult>(){}.getType());
