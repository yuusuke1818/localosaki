...
public class AddBasicFVPaSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200018_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<AddBasicFVPaSelectResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<AddBasicFVPaSelectResult>>(){}.getType());
		AddBasicFVPaSelectResult paramResult = param.getResult();
		...
		AddBasicFVPaSelectResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<AddBasicFVPaSelectResult>(){}.getType());
