...
import jp.co.osaki.osol.api.result.smcontrol.extract.LightingOptionSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class LightingOptionSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200048_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<LightingOptionSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<LightingOptionSelectExtractResult>>(){}.getType());
		LightingOptionSelectExtractResult paramResult = param.getResult();
		...
		LightingOptionSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<LightingOptionSelectExtractResult>(){}.getType());
