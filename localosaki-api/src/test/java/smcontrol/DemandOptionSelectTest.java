...
import jp.co.osaki.osol.api.result.smcontrol.extract.DemandOptionSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class DemandOptionSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200030_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<DemandOptionSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<DemandOptionSelectExtractResult>>(){}.getType());
		DemandOptionSelectExtractResult paramResult = param.getResult();
		...
		DemandOptionSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<DemandOptionSelectExtractResult>(){}.getType());
