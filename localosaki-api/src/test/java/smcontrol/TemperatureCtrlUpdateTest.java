...
import jp.co.osaki.osol.api.result.smcontrol.extract.TemperatureCtrlSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class TemperatureCtrlUpdateTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200003_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<TemperatureCtrlSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<TemperatureCtrlSelectExtractResult>>(){}.getType());
		TemperatureCtrlSelectExtractResult paramResult = param.getResult();
		...
		TemperatureCtrlSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<TemperatureCtrlSelectExtractResult>(){}.getType());
