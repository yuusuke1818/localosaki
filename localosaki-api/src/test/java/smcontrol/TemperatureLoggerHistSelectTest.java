...
import jp.co.osaki.osol.api.result.smcontrol.extract.TemperatureLoggerHistSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class TemperatureLoggerHistSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200037_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<TemperatureLoggerHistSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<TemperatureLoggerHistSelectExtractResult>>(){}.getType());
		TemperatureLoggerHistSelectExtractResult paramResult = param.getResult();
		...
		TemperatureLoggerHistSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<TemperatureLoggerHistSelectExtractResult>(){}.getType());
