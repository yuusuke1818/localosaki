...
import jp.co.osaki.osol.api.result.smcontrol.extract.RadioWaveStateFOMASelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class RadioWaveStateFOMASelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200033_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<RadioWaveStateFOMASelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<RadioWaveStateFOMASelectExtractResult>>(){}.getType());
		RadioWaveStateFOMASelectExtractResult paramResult = param.getResult();
		...
		RadioWaveStateFOMASelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<RadioWaveStateFOMASelectExtractResult>(){}.getType());
