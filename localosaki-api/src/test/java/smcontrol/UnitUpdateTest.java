...
import jp.co.osaki.osol.api.result.smcontrol.extract.UnitSelectExtractResult;
...
public class UnitUpdateTest {
	...
	private static String API_ID = SmConstants.A200013_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<UnitSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<UnitSelectExtractResult>>(){}.getType());
		UnitSelectExtractResult paramResult = param.getResult();
		...
		UnitSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<UnitSelectExtractResult>(){}.getType());
