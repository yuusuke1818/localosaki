...
import jp.co.osaki.osol.api.result.smcontrol.extract.LoadCtrlResultSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class LoadCtrlResultSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200038_TEST;
	...
	public void test_001() {
		...
		try {
			...
			String strTargetDate = inRb.getString(SmConstants.PARAM_TARGET_DATE);
			String strSelectTarget = inRb.getString(SmConstants.PARAM_SELECT_TARGET);
			String strHist = inRb.getString(SmConstants.PARAM_HIST);

			// リクエスト生成
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					SmConstants.PARAM_SMID + "=" + strSmId + "&" +
					SmConstants.PARAM_TARGET_DATE + "=" + strTargetDate + "&" +
					SmConstants.PARAM_SELECT_TARGET + "=" + strSelectTarget + "&" +
					SmConstants.PARAM_HIST + "=" + strHist;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<LoadCtrlResultSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<LoadCtrlResultSelectExtractResult>>(){}.getType());
		LoadCtrlResultSelectExtractResult paramResult = param.getResult();
		...
		LoadCtrlResultSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<LoadCtrlResultSelectExtractResult>(){}.getType());
