...
public class PingSelectTest {
	...
	public void test_001() {
		...
		try {
			...
			String strIpAddress = inRb.getString(SmConstants.PARAM_IP_ADDRESS);
			String strCount = inRb.getString(SmConstants.PARAM_COUNT);

			// リクエスト生成
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					SmConstants.PARAM_IP_ADDRESS + "=" + strIpAddress + "&" +
					SmConstants.PARAM_COUNT + "=" + strCount;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<PingSelectResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<PingSelectResult>>(){}.getType());
		PingSelectResult paramResult = param.getResult();
		...
		PingSelectResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<PingSelectResult>(){}.getType());
		...
		if (!(expected == null)) {
			assertReflectionEquals(expected.getCommunicationResultCd() , paramResult.getCommunicationResultCd());
		}else {
