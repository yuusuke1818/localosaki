...
import jp.co.osaki.osol.api.result.smcontrol.extract.CommunicationOpModeSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.TSmStub;

public class CommunicationOpModeSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static TSmStub stub;

	private static String API_ID = SmConstants.A200016_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<CommunicationOpModeSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<CommunicationOpModeSelectExtractResult>>(){}.getType());
		CommunicationOpModeSelectExtractResult paramResult = param.getResult();
		...
		CommunicationOpModeSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<CommunicationOpModeSelectExtractResult>(){}.getType());
