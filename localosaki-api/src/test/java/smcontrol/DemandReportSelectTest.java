...
import jp.co.osaki.osol.api.result.smcontrol.extract.DemandReportSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class DemandReportSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200029_TEST;
	...
	public void test_001() {
		...
		try {
			...
			String reportHist = inRb.getString(SmConstants.PARAM_REPORT_HIST);

			// リクエスト生成
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					SmConstants.PARAM_SMID + "=" + strSmId + "&" +
					SmConstants.PARAM_REPORT_HIST + "=" + reportHist;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<DemandReportSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<DemandReportSelectExtractResult>>(){}.getType());
		DemandReportSelectExtractResult paramResult = param.getResult();
		...
		DemandReportSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<DemandReportSelectExtractResult>(){}.getType());
