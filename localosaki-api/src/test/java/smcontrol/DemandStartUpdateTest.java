...
public class DemandStartUpdateTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200025_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<DemandStartUpdateResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<DemandStartUpdateResult>>(){}.getType());
		DemandStartUpdateResult paramResult = param.getResult();
		...
		DemandStartUpdateResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<DemandStartUpdateResult>(){}.getType());
