...
import jp.co.osaki.osol.api.result.smcontrol.extract.DeviceInfoSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class DeviceInfoSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200034_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<DeviceInfoSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<DeviceInfoSelectExtractResult>>(){}.getType());
		DeviceInfoSelectExtractResult paramResult = param.getResult();
		...
		DeviceInfoSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<DeviceInfoSelectExtractResult>(){}.getType());
