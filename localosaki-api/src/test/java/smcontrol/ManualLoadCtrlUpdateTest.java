...
import jp.co.osaki.osol.api.result.smcontrol.extract.ManualLoadCtrlSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class ManualLoadCtrlUpdateTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200009_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<ManualLoadCtrlSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<ManualLoadCtrlSelectExtractResult>>(){}.getType());
		ManualLoadCtrlSelectExtractResult paramResult = param.getResult();
		...
		ManualLoadCtrlSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<ManualLoadCtrlSelectExtractResult>(){}.getType());
