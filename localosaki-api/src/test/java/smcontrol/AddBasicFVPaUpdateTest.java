...
public class AddBasicFVPaUpdateTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200019_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<AddBasicFVPaUpdateResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<AddBasicFVPaUpdateResult>>(){}.getType());
		AddBasicFVPaUpdateResult paramResult = param.getResult();
		...
		AddBasicFVPaUpdateResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<AddBasicFVPaUpdateResult>(){}.getType());
