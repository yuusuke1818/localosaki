...
public class SendArbitraryTelegramTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200026_TEST;
	...
	public void test_001() {
		...
		try {
			...
			String strAnyMessage = inRb.getString(SmConstants.PARAM_ANY_MESSAGE);


			// リクエスト生成
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					SmConstants.PARAM_SMID + "=" + strSmId + "&" +
					SmConstants.PARAM_ANY_MESSAGE + "=" + strAnyMessage;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<SendArbitraryTelegramResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<SendArbitraryTelegramResult>>(){}.getType());
		SendArbitraryTelegramResult paramResult = param.getResult();
		...
		SendArbitraryTelegramResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<SendArbitraryTelegramResult>(){}.getType());
