...
import jp.co.osaki.osol.api.result.smcontrol.extract.ContinuousMonitorringSelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class ContinuousMonitorringSelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200031_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<ContinuousMonitorringSelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<ContinuousMonitorringSelectExtractResult>>(){}.getType());
		ContinuousMonitorringSelectExtractResult paramResult = param.getResult();
		...
		ContinuousMonitorringSelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<ContinuousMonitorringSelectExtractResult>(){}.getType());
