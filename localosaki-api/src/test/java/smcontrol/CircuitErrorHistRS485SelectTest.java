...
import jp.co.osaki.osol.api.result.smcontrol.extract.CircuitErrorHistRS485SelectExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class CircuitErrorHistRS485SelectTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200036_TEST;
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<CircuitErrorHistRS485SelectExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<CircuitErrorHistRS485SelectExtractResult>>(){}.getType());
		CircuitErrorHistRS485SelectExtractResult paramResult = param.getResult();
		...
		CircuitErrorHistRS485SelectExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<CircuitErrorHistRS485SelectExtractResult>(){}.getType());
