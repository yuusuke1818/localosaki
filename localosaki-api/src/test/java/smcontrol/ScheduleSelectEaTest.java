...
import jp.co.osaki.osol.api.result.smcontrol.extract.ScheduleSelectEaExtractResult;
import util.SmConstants;
import util.SmControlTestUtil;
import util.SmStub;

public class ScheduleSelectEaTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200059_TEST;
	...
	public void test_001() {
		...
		try {
			...
			String scheduleControlInfo = inRb.getString(SmConstants.PARAM_SCHEDULE_CONTROL_INFO);
			...
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					...
					SmConstants.PARAM_SCHEDULE_CONTROL_INFO + "=" + scheduleControlInfo + "&" +
	...
	private void commonExecute(String strParam){
		...
		// レスポンスからGsonパースで値セット
		SmControlApiResponse<ScheduleSelectEaExtractResult> param = new Gson().fromJson(output, new TypeToken<SmControlApiResponse<ScheduleSelectEaExtractResult>>(){}.getType());
		ScheduleSelectEaExtractResult paramResult = param.getResult();
		...
		ScheduleSelectEaExtractResult expected = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<ScheduleSelectEaExtractResult>(){}.getType());
