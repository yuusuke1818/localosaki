...
public class BulkCtrlTest {

	private static SmControlTestUtil util = new SmControlTestUtil();

	private static SmStub stub;

	private static String API_ID = SmConstants.A200047_TEST;

	private String METHOD_NAME = "";

	@BeforeClass
	public static void setUpBeforeClass() throws Exception {
		// クラスの開始で呼び出すか、メソッドの開始で呼び出すか。
	...
	public void test_001() {
		...
		try {
			...
			String strTargetPowerList = inRb.getString(SmConstants.PARAM_TARGET_POWER_LIST);

			// リクエスト生成
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					SmConstants.PARAM_TARGET_POWER_LIST + "=" + strTargetPowerList;
	...
	public void test_003() {
		...
		try {
			...
			// プロパティファイルから値取得
			String strBean = inRb.getString(SmConstants.PARAM_BEAN);
			String strCoolingList = inRb.getString(SmConstants.PARAM_COOLING_LIST);

			// リクエスト生成
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					SmConstants.PARAM_COOLING_LIST + "=" + strCoolingList;
	...
	public void test_004() {
		...
		try {
			...
			// プロパティファイルから値取得
			String strBean = inRb.getString(SmConstants.PARAM_BEAN);
			String strHeatingList = inRb.getString(SmConstants.PARAM_HEATING_LIST);

			// リクエスト生成
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					SmConstants.PARAM_HEATING_LIST + "=" + strHeatingList;
	...
	public void test_005() {
		...
		try {
			...
			// プロパティファイルから値取得
			String strBean = inRb.getString(SmConstants.PARAM_BEAN);
			String strScheduleList = inRb.getString(SmConstants.PARAM_SCHEDULE_LIST);

			// リクエスト生成
			String strParam = SmConstants.PARAM_BEAN + "=" + strBean + "&" +
					SmConstants.PARAM_SCHEDULE_LIST + "=" + strScheduleList;
	...
	private void commonExecute(String strParam){
		...
		List<BulkApiResultData<BulkTargetPowerExtractResult>> bulkResultList = param.getResultList();
		...
		SmControlApiResponse<BulkTargetPowerExtractResult> expected = null;
		List<BulkApiResultData<BulkTargetPowerExtractResult>> expectedList = null;
		...
		try {
			...
			if(!(outRb.getString(SmConstants.JSON_RESULT) == null) ) {
				expected =
						new Gson().fromJson(outRb.getString(SmConstants.JSON_RESULT), new TypeToken<SmControlApiResponse<BulkTargetPowerExtractResult>>(){}.getType());

				expectedList = expected.getResultList();
		...
		if (!(expected == null)) {
			assertReflectionEquals(expectedList , bulkResultList);
		}else {
			assertThat(bulkResultList, is(nullValue()));
		}
