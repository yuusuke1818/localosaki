...
/**
 * デマンドデータ実績取得処理（全体・建物・テナント一覧・削減率（導入前比）） Daoクラス
 *
 * @author ya-ishida
 */
@Stateless
public class DemandAllSummaryBuildingReductionInnovateDataDao
        extends OsolApiDao<DemandAllSummaryBuildingReductionInnovateDataParameter> {
    ...
    public DemandAllSummaryBuildingReductionInnovateDataDao() {
     ...
     */
    @Override
    public DemandAllSummaryBuildingReductionInnovateDataResult query(
            DemandAllSummaryBuildingReductionInnovateDataParameter parameter) throws Exception {
        DemandAllSummaryBuildingReductionInnovateDataResult result = new DemandAllSummaryBuildingReductionInnovateDataResult();
        ...
        if (exCorpList == null || exCorpList.size() != 1) {
            return new DemandAllSummaryBuildingReductionInnovateDataResult();
        }
        ...
        for (AllBuildingListDetailResultData buildingResult : buildingList) {
            ...
            if (targetBeforeWeek != null) {
                ...
                //デマンド系統週報テーブルから年度0の指定週のデータを取得する（導入前基準値）
                //導入前データは企業集計のみ
                CommonDemandWeekReportLineListResult weekLineCompareParam = DemandEmsUtility.getWeekReportLineListParam(
                        ...
                        ApiGenericTypeConstants.LINE_TARGET.ALL.getVal(), "0", "0", targetBeforeWeek.getWeekNo(),
                        targetBeforeWeek.getWeekNo());
                beforeWeekCompareResult = getResultList(commonDemandWeekReportLineListServiceDaoImpl,
                        weekLineCompareParam);
        ...
        if (!CheckUtility.isNullOrEmpty(parameter.getSortKey())
            ...
            if (!detail.isEmpty()) {
                detail = SortUtility.sortDemandAllBuildingList(detail,
                        ApiCodeValueConstants.ALL_SUMMARY_DETAIL_KIND.REDUCTION_BEFORE_INNOVATE.getVal(),
