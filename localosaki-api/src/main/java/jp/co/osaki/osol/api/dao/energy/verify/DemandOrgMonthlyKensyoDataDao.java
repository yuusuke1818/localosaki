...
import jp.co.osaki.osol.api.resultdata.energy.verify.DemandOrgMonthlyKensyoDataDetailResultData;
import jp.co.osaki.osol.api.resultdata.energy.verify.DemandOrgMonthlyKensyoDataHeaderResultData;
 ...
 */
@Stateless
public class DemandOrgMonthlyKensyoDataDao extends OsolApiDao<DemandOrgMonthlyKensyoDataParameter> {
    ...
    public DemandOrgMonthlyKensyoDataDao() {
     ...
     */
    @Override
    public DemandOrgMonthlyKensyoDataResult query(DemandOrgMonthlyKensyoDataParameter parameter) throws Exception {
        DemandOrgMonthlyKensyoDataResult result = new DemandOrgMonthlyKensyoDataResult();
        DemandOrgMonthlyKensyoDataHeaderResultData headerResultSet = new DemandOrgMonthlyKensyoDataHeaderResultData();
        List<DemandOrgMonthlyKensyoDataDetailResultData> detailList = new ArrayList<>();
        List<DemandOrgMonthlyKensyoDataTimeDetailResultData> timeDetailActualList;
        List<DemandOrgMonthlyKensyoDataTimeDetailResultData> timeDetailPredictionList;
        List<DemandOrgMonthlyKensyoDataTimeDetailResultData> timeDetailReductionList;
        Date currentDate;
        BigDecimal controlCount;
        ...
        if (smList == null || smList.size() != 1) {
            return new DemandOrgMonthlyKensyoDataResult();
        }
        ...
        //時間帯別実績情報を作成する
        timeDetailActualList = DemandDataUtility.createMonthlyKensyoDetailHeader(summaryRange);
        timeDetailPredictionList = DemandDataUtility.createMonthlyKensyoDetailHeader(summaryRange);
        timeDetailReductionList = DemandDataUtility.createMonthlyKensyoDetailHeader(summaryRange);
        ...
        while (!currentDate.after(summaryRange.getMeasurementDateTo())) {
            ...
            if (dailyKensyoDataResultSet == null || dailyKensyoDataResultSet.getHeader() == null
                    || dailyKensyoDataResultSet.getDetailList() == null
                    || dailyKensyoDataResultSet.getDetailList().isEmpty()) {
                //日報情報が取得できない場合は1日進めて次のレコードへ
            ...
            if (dailyKensyoDataResultSet != null) {
                ...
                if (dailyKensyoDataResultSet.getHeader().getControlCount() != null) {
                    if (controlCount == null) {
                        controlCount = dailyKensyoDataResultSet.getHeader().getControlCount();
                    } else {
                        controlCount = controlCount.add(dailyKensyoDataResultSet.getHeader().getControlCount());
                    }
                }

                // 実績値
                if (dailyKensyoDataResultSet.getHeader().getMaxDemandValue() != null) {
                    if (maxActual == null) {
                        maxActual = dailyKensyoDataResultSet.getHeader().getMaxDemandValue();
                    } else if (dailyKensyoDataResultSet.getHeader().getMaxDemandValue().compareTo(maxActual) > 0) {
                        maxActual = dailyKensyoDataResultSet.getHeader().getMaxDemandValue();
                    }

                    if (minActual == null) {
                        minActual = dailyKensyoDataResultSet.getHeader().getMaxDemandValue();
                    } else if (dailyKensyoDataResultSet.getHeader().getMaxDemandValue().compareTo(minActual) < 0) {
                        minActual = dailyKensyoDataResultSet.getHeader().getMaxDemandValue();
                    }

                    if (summaryActual == null) {
                        summaryActual = dailyKensyoDataResultSet.getHeader().getMaxDemandValue();
                    } else {
                        summaryActual = summaryActual.add(dailyKensyoDataResultSet.getHeader().getMaxDemandValue());
                    }
                ...
                for (DemandOrgMonthlyKensyoDataTimeDetailResultData timeDetailActual : timeDetailActualList) {
                    if (timeDetailActual.getMeasurementDate().equals(currentDate)) {
                        timeDetailActual.setResultValue(dailyKensyoDataResultSet.getHeader().getMaxDemandValue());
                    }
                }

                //予測値
                if (dailyKensyoDataResultSet.getHeader().getMaxPredictionValue() != null) {
                    if (maxPrediction == null) {
                        maxPrediction = dailyKensyoDataResultSet.getHeader().getMaxPredictionValue();
                    } else if (dailyKensyoDataResultSet.getHeader().getMaxPredictionValue()
                            .compareTo(maxPrediction) > 0) {
                        maxPrediction = dailyKensyoDataResultSet.getHeader().getMaxPredictionValue();
                    }

                    if (minPrediction == null) {
                        minPrediction = dailyKensyoDataResultSet.getHeader().getMaxPredictionValue();
                    } else if (dailyKensyoDataResultSet.getHeader().getMaxPredictionValue()
                            .compareTo(minPrediction) < 0) {
                        minPrediction = dailyKensyoDataResultSet.getHeader().getMaxPredictionValue();
                    }

                    if (summaryPrediction == null) {
                        summaryPrediction = dailyKensyoDataResultSet.getHeader().getMaxPredictionValue();
                    } else {
                        summaryPrediction = summaryPrediction
                                .add(dailyKensyoDataResultSet.getHeader().getMaxPredictionValue());
                    }
                    ...
                    for (DemandOrgMonthlyKensyoDataTimeDetailResultData timeDetailPrediction : timeDetailPredictionList) {
                        if (timeDetailPrediction.getMeasurementDate().equals(currentDate)) {
                            timeDetailPrediction
                                    .setResultValue(dailyKensyoDataResultSet.getHeader().getMaxPredictionValue());
                        }
                    }
                }

                //削減値
                if (dailyKensyoDataResultSet.getHeader().getMaxReductionValue() != null) {
                    if (maxReduction == null) {
                        maxReduction = dailyKensyoDataResultSet.getHeader().getMaxReductionValue();
                    } else if (dailyKensyoDataResultSet.getHeader().getMaxReductionValue()
                            .compareTo(maxReduction) > 0) {
                        maxReduction = dailyKensyoDataResultSet.getHeader().getMaxReductionValue();
                    }

                    if (minReduction == null) {
                        minReduction = dailyKensyoDataResultSet.getHeader().getMaxReductionValue();
                    } else if (dailyKensyoDataResultSet.getHeader().getMaxReductionValue()
                            .compareTo(minReduction) < 0) {
                        minReduction = dailyKensyoDataResultSet.getHeader().getMaxReductionValue();
                    }

                    if (summaryReduction == null) {
                        summaryReduction = dailyKensyoDataResultSet.getHeader().getMaxReductionValue();
                    } else {
                        summaryReduction = summaryReduction
                                .add(dailyKensyoDataResultSet.getHeader().getMaxReductionValue());
                    }
                    ...
                    for (DemandOrgMonthlyKensyoDataTimeDetailResultData timeDetailReduction : timeDetailReductionList) {
                        if (timeDetailReduction.getMeasurementDate().equals(currentDate)) {
                            timeDetailReduction
                                    .setResultValue(dailyKensyoDataResultSet.getHeader().getMaxReductionValue());
                        }
                    }
                }
            }
        ...
        //結果値の小数第2位を切り捨てする
        //予測値
        for (DemandOrgMonthlyKensyoDataTimeDetailResultData timeDetail : timeDetailPredictionList) {
        ...
        //削減値
        for (DemandOrgMonthlyKensyoDataTimeDetailResultData timeDetail : timeDetailReductionList) {
            if (timeDetail.getResultValue() != null) {
                timeDetail.setResultValue(timeDetail.getResultValue().setScale(1, BigDecimal.ROUND_DOWN));
            }
        }
        ...
        //実績値を詰める
        DemandOrgMonthlyKensyoDataDetailResultData detailActual = new DemandOrgMonthlyKensyoDataDetailResultData();
        ...
        //予測値を詰める
        DemandOrgMonthlyKensyoDataDetailResultData detailPrediction = new DemandOrgMonthlyKensyoDataDetailResultData();
        ...
        //削減値を詰める
        DemandOrgMonthlyKensyoDataDetailResultData detailReduction = new DemandOrgMonthlyKensyoDataDetailResultData();
        ...
        //結果を詰める
        result.setHeader(headerResultSet);
    ...
    private DemandOrgDailyKensyoDataResult getDemandOrgDailyKensyoDataResultSet(
            DemandOrgMonthlyKensyoDataParameter parameter, Date currentDate, Date serverDateTime,
            ProductControlLoadFlgSeparateListResult productControlLoadData, boolean beforeG2Flg)
            throws ParseException {
