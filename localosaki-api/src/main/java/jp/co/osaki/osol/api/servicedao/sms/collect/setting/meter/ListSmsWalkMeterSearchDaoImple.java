...
/**
 * メーター管理 ハンディ検針用メーター情報一覧取得 ServiceDaoクラス
 * @author sagi_h
 *
 */
public class ListSmsWalkMeterSearchDaoImple implements BaseServiceDao<ListSmsMeterResultData> {
    ...
    public List<ListSmsMeterResultData> getResultList(ListSmsMeterResultData target, EntityManager em) {
        ...
        // 建物メーター中間テーブルとの関連(装置ID、メーター管理番号、所属建物除外)
        ...
        // MM.DEV_ID = 'M388819Y' /* $2 */ and SUBSTR(MM.DEV_ID, 1, 2) = 'MH'
        whereList.add(builder.equal(root.get(MMeter_.id).get(MMeterPK_.devId), target.getDevId()));
        whereList.add(builder.like(root.get(MMeter_.id).get(MMeterPK_.devId), DEVICE_KIND.HANDY.getVal() + "%"));
    	...
    	query = query.select(builder.construct(ListSmsMeterResultData.class,
                ...
                root.get(MMeter_.wirelessType),
                root.get(MMeter_.wirelessId),
                root.get(MMeter_.pollingId),
                root.get(MMeter_.hop1Id),
                root.get(MMeter_.hop2Id),
                root.get(MMeter_.hop3Id),
                root.get(MMeter_.version))).distinct(true).where(builder.and(whereList.toArray(new Predicate[] {})));
