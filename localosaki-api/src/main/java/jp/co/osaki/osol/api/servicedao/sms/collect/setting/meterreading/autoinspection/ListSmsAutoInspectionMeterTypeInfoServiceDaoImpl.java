...
/**
 * データ収集装置 機器管理 検針設定 自動検針画面 データ取得API メーター種別情報取得ServiceDaoクラス.
 *
 * @author ozaki.y
 */
public class ListSmsAutoInspectionMeterTypeInfoServiceDaoImpl
        implements BaseServiceDao<ListSmsAutoInspectionResultData> {
    ...
    public List<ListSmsAutoInspectionResultData> getResultList(ListSmsAutoInspectionResultData target,
        ...
        return selectMeterTypeInfoList(em, target.getCorpId(), target.getBuildingId(), target.isTenant());
    }
     ...
     * @return メーター種別情報List
     */
    private List<ListSmsAutoInspectionResultData> selectMeterTypeInfoList(EntityManager em, String corpId,
        ...
        Root<MMeterType> mMeterType = query.from(MMeterType.class);

        Path<String> corpIdPath = mMeterType.get(MMeterType_.id).get(MMeterTypePK_.corpId);
        Path<Long> buildingIdPath = mMeterType.get(MMeterType_.id).get(MMeterTypePK_.buildingId);
        ...
        Expression<?>[] targetColumns = new Expression[] { corpIdPath, buildingIdPath, meterTypePath,
                meterTypeNamePath };
