...
import jp.co.osaki.osol.api.request.energy.verify.EventVerifySettingUpdateRequest;
 ...
 */
@Stateless
public class EventVerifySettingUpdateDao extends OsolApiDao<EventVerifySettingUpdateParameter> {
    ...
    public EventVerifySettingUpdateDao() {
    ...
    @Override
    public EventVerifySettingUpdateResult query(EventVerifySettingUpdateParameter parameter) throws Exception {

        MCorp exCorp;
        TBuilding exBuilding;
        MSmPrm exSm;
        List<MSmControlLoad> smControlLoadList;
        List<MSmControlLoadVerify> smControlLoadVerifyList;
        ...
        if (parameter.getResultSet() == null) {
            return new EventVerifySettingUpdateResult();
        }

        //JSON⇒Resultに変換
        EventVerifySettingUpdateRequest resultSet = parameter.getResultSet();
        ...
        //機器制御負荷検証の排他チェック
        smControlLoadVerifyList = smControlLoadVerifyExclusiveCheck(resultSet.getSmControlLoadVerifyList());
        ...
        updateSmControlLoad(resultSet.getSmControlLoadList(), resultSet.getProductControlLoadList(), smControlLoadList,
                resultSet.getSmControlLoadVerifyList(), resultSet.getSmLineControlLoadVerifyList(), parameter.getSmId(),
                serverDateTime,
                loginUserId);
        //機器制御負荷検証の更新
        updateSmControlLoadVerify(resultSet.getSmControlLoadVerifyList(), smControlLoadVerifyList, serverDateTime,
                loginUserId);
        //機器系統検証の更新
     ...
     * @return
     */
    private MCorp corpExclusiveCheck(EventVerifySettingUpdateParameter parameter) throws Exception {
     ...
     * @return
     */
    private TBuilding buildingExclusiveCheck(EventVerifySettingUpdateParameter parameter) throws Exception {
     ...
     * @return
     */
    private MSmPrm smExclusiveCheck(EventVerifySettingUpdateParameter parameter) throws Exception {
     ...
     * @throws Exception
     */
    private List<MSmControlLoadVerify> smControlLoadVerifyExclusiveCheck(
            List<SmControlLoadVerifyListDetailResultData> updateList) throws Exception {

        List<MSmControlLoadVerify> resultList = new ArrayList<>();

        if (updateList == null || updateList.isEmpty()) {
            return resultList;
        }

        for (SmControlLoadVerifyListDetailResultData smControlLoadVerify : updateList) {
            //機器制御負荷検証の排他チェック
            MSmControlLoadVerify param = new MSmControlLoadVerify();
            MSmControlLoadVerifyPK pkParam = new MSmControlLoadVerifyPK();
            pkParam.setSmId(smControlLoadVerify.getSmId());
            pkParam.setControlLoad(smControlLoadVerify.getControlLoad());
            param.setId(pkParam);
            MSmControlLoadVerify updateData = find(mSmControlLoadVerifyServiceDaoImpl, param);
            if (smControlLoadVerify.getVersion() == null) {
                //新規登録データの場合
                if (updateData != null) {
                    //すでに登録済のデータがある場合、排他エラー
                    throw new OptimisticLockException();
                ...
                //更新データの場合
                if (updateData == null || !updateData.getVersion().equals(smControlLoadVerify.getVersion())) {
                    //排他制御のデータがない場合または前に保持していたVersionと異なる場合、排他エラー
                    throw new OptimisticLockException();
                } else {
                    resultList.add(updateData);
    ...
    private void updateSmControlLoad(List<SmControlLoadListDetailResultData> updateList,
            ...
            List<SmControlLoadVerifyListDetailResultData> smControlLoadVerifyList,
        ...
        //①機器制御負荷に更新対象がないが、機器制御負荷検証に対象データがある場合
        if (smControlLoadVerifyList != null && !smControlLoadVerifyList.isEmpty()) {
            for (SmControlLoadVerifyListDetailResultData smControlLoadVerify : smControlLoadVerifyList) {
                Boolean existFlg = Boolean.FALSE;
                for (SmControlLoadListDetailResultData updateData : updateList) {
                    if (smControlLoadVerify.getSmId().equals(updateData.getSmId())
                            && smControlLoadVerify.getControlLoad().compareTo(updateData.getControlLoad()) == 0) {
                        //一致する場合は対応不要
                ...
                if (!existFlg) {
                    //存在しない場合のみ対象に追加
                    if (!registControlLoadList.contains(smControlLoadVerify.getControlLoad())) {
                        //一致する制御負荷がリストにない場合のみ追加
                        registControlLoadList.add(smControlLoadVerify.getControlLoad());
                    }
                }
            }
        }
     ...
     * @throws Exception
     */
    private void updateSmControlLoadVerify(List<SmControlLoadVerifyListDetailResultData> updateList,
            List<MSmControlLoadVerify> entityList, Timestamp serverDateTime, Long loginUserId) throws Exception {
        ...
        //更新対象データの登録
        int i = 0;
        for (SmControlLoadVerifyListDetailResultData updateData : updateList) {
            MSmControlLoadVerify updateParam = new MSmControlLoadVerify();
            MSmControlLoadVerifyPK pkUpdateParam = new MSmControlLoadVerifyPK();
            Boolean newFlg = Boolean.FALSE;
            ...
            updateParam.setControlLoadRunningHours(updateData.getControlLoadRunningHours());
            updateParam.setDelFlg(updateData.getDelFlg());
            updateParam.setUpdateDate(serverDateTime);
            updateParam.setUpdateUserId(loginUserId);

            if (newFlg) {
                persist(mSmControlLoadVerifyServiceDaoImpl, updateParam);
            } else {
                merge(mSmControlLoadVerifyServiceDaoImpl, updateParam);
            }
     ...
     * @return
     */
    private EventVerifySettingUpdateResult getUpdateData(EventVerifySettingUpdateParameter parameter) {

        EventVerifySettingUpdateResult result = new EventVerifySettingUpdateResult();
