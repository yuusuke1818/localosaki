...
/**
 *
 *集計分析 EMS実績 対象建物取得(建物ID指定) ServiceDaoクラス
 *
 * @author y-maruta
 *
 */

public class AnalysisBuildingIdListServiceDaoImpl implements BaseServiceDao<AnalysisAllBuildingListResultData> {
        ...
        if(belongTenantFlg) {
            ...
            whereList.add(
                    builder.or(
                            builder.and(
                                    //企業ID
                                    builder.equal(root.get(TBuilding_.id).get(TBuildingPK_.corpId), corpId)
                                    //建物ID
                                    ,builder.equal(root.get(TBuilding_.id).get(TBuildingPK_.buildingId), buildingId)
                                    )
                            , builder.and(
                                    //所属企業ID
                                    builder.equal(root.get(TBuilding_.divisionCorpId), corpId)
                                    //所属建物ID
                                    ,builder.equal(root.get(TBuilding_.divisionBuildingId), buildingId))
                    )
             );
        }else {
