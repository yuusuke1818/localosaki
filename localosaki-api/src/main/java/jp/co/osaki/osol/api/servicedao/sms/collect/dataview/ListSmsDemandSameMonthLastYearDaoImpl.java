...
public class ListSmsDemandSameMonthLastYearDaoImpl implements BaseServiceDao<ListSmsDataViewResultData> {
    ...
    public List<ListSmsDataViewResultData> getResultList(ListSmsDataViewResultData target, EntityManager em) {
        ...
        if (target.isForwardDiract()) {
            // 正方向の場合
            query = createForwardDiractQuery(builder, devId, meterMngIdClct, dayOfMonthList);
        } else {
            // 逆方向の場合
            query = createReverseDiractQuery(builder, devId, meterMngIdClct, dayOfMonthList);
        }
    ...
    private CriteriaQuery<ListSmsDataViewResultData> createForwardDiractQuery(CriteriaBuilder builder, String devId,
            Collection<Long> meterMngIdClct, List<String> dayOfMonthList) {
        int size = dayOfMonthList.size();
        String startYear = DateUtility.plusYear(dayOfMonthList.get(0).substring(0, 4), -1);
        String endYear = DateUtility.plusYear(dayOfMonthList.get(size - 1).substring(0, 4), -1);
        ...
        if(dayOfMonthList.size() > 27) {
            startDate = startYear + dayOfMonthList.get(0).substring(4, 8);
            endDate = endYear + dayOfMonthList.get(size - 1).substring(4, 8);
        }else {
            // 年報表示のとき
            startDate = startYear + dayOfMonthList.get(0).substring(4, 8);
            endDate = endYear + dayOfMonthList.get(size - 1).substring(4, 6) + "31";
        }
        ...
        .groupBy(root.get(TDayLoadSurveyMaxDemand_.id).get(TDayLoadSurveyMaxDemandPK_.meterMngId));
    ...
    private CriteriaQuery<ListSmsDataViewResultData> createReverseDiractQuery(CriteriaBuilder builder, String devId,
        ...
        .where(whereList.toArray(new Predicate[]{}))
        .groupBy(root.get(TDayLoadSurveyMaxDemandRev_.id).get(TDayLoadSurveyMaxDemandRevPK_.meterMngId));
