...
import jp.co.osaki.osol.api.dao.smcontrol.UnitUpdateEaDao;
import jp.co.osaki.osol.api.parameter.smcontrol.UnitUpdateEaParameter;
import jp.co.osaki.osol.api.result.smcontrol.SmPointResult;
import jp.co.osaki.osol.api.result.smcontrol.UnitUpdateEaResult;
...
import jp.co.osaki.osol.mng.param.A200054Param;
...
@Named(value = SmControlConstants.UNIT_UPDATE_EA)
@RequestScoped
public class UnitUpdateEaBean
extends AbstractApiBean<UnitUpdateEaResult, UnitUpdateEaParameter> {

    @EJB
    private UnitUpdateEaDao dao;
    ...
    @Override
    protected <T extends BaseParam> T initParam(UnitUpdateEaParameter parameter) {

        A200054Param param = new Gson().fromJson(parameter.getResult(), A200054Param.class);
    ...
    protected void callDao(FvpCtrlMngResponse<?> response) throws Exception {
        ...
        // Paramクラス変更
        @SuppressWarnings("unchecked")
        FvpCtrlMngResponse<A200054Param> res = (FvpCtrlMngResponse<A200054Param>) response;
        A200054Param param = res.getParam();
