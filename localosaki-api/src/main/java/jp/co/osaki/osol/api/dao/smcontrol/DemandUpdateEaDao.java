 ...
 */
@Stateless
public class DemandUpdateEaDao extends BaseSmControlDao {
    ...
    // DAO内に処理を記載
    public void updateBuildingDM(BuildingDmResult param, FvpCtrlMngResponse<A200050Param> res) throws Exception {
     ...
     * @param updateDate
     */
    private void updateSmControlLoad(FvpCtrlMngResponse<A200050Param> res, Long loginUserId, Timestamp updateDate) {
        //負荷情報リスト
        List<Map<String, String>> loadInfoList = res.getParam().getLoadInfoList();
        //機器ID
        Long smId = res.getSmId();

        //インデックス
        int controlLoadIndex = 0;
        for (Map<String, String> loadInfo : loadInfoList) {
            //最小負荷遮断順位（遮断方法+遮断順位）
            String shutOffRank = null;
            //最小負荷遮断時間
            String minLoadBlockTime = loadInfo.get("minLoadBlockTime");
            //遮断方法
            String loadBlockMethod = loadInfo.get("loadBlockMethod");
            //遮断順位
            String loadBlockRank = loadInfo.get("loadBlockRank");
            //負荷遮断容量
            String shutOffCapacity = loadInfo.get("loadBlockCapacity");
