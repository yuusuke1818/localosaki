...
import jp.co.osaki.osol.api.resultdata.energy.ems.DemandOrgYearReportListLineMaxValDetailResultData;
import jp.co.osaki.osol.api.resultdata.energy.ems.DemandOrgYearReportListLineMaxValHeaderResultData;
import jp.co.osaki.osol.api.resultdata.energy.ems.DemandOrgYearReportListLineMaxValHeaderTitleResultData;
import jp.co.osaki.osol.api.resultdata.energy.ems.DemandOrgYearReportListLineMaxValSummaryAreaResultData;
...
/**
 * エネルギー使用状況実績取得（個別・年報・系統別最大値） Daoクラス
 *
 * @author ya-ishida
 */
@Stateless
public class DemandOrgYearReportListLineMaxValDao extends OsolApiDao<DemandOrgYearReportListLineMaxValParameter> {
    ...
    public DemandOrgYearReportListLineMaxValDao() {
    ...
    @Override
    public DemandOrgYearReportListLineMaxValResult query(DemandOrgYearReportListLineMaxValParameter parameter)
            throws Exception {
        DemandOrgYearReportListLineMaxValResult result = new DemandOrgYearReportListLineMaxValResult();
        DemandOrgYearReportListLineMaxValHeaderResultData header = new DemandOrgYearReportListLineMaxValHeaderResultData();
        List<DemandOrgYearReportListLineMaxValHeaderTitleResultData> headerTitleList = new ArrayList<>();
        List<DemandOrgYearReportListLineMaxValHeaderTitleResultData> headerTitleListForAll = new ArrayList<>();
        List<DemandOrgYearReportListLineMaxValDetailResultData> detailList = new ArrayList<>();
        DemandOrgYearReportListLineMaxValSummaryAreaResultData summary = new DemandOrgYearReportListLineMaxValSummaryAreaResultData();
        ...
        List<BigDecimal> elementNumMaxListForAll = new ArrayList<>();
        BigDecimal contractPowerMax = null;
        BigDecimal targetPowerMax = null;
        ...
        if (ApiGenericTypeConstants.SUMMARY_UNIT.CORP.getVal().equals(parameter.getSummaryKind())) {
            ...
            if (corpDemandList == null || corpDemandList.size() != 1) {
                return new DemandOrgYearReportListLineMaxValResult();
            }
        ...
        do {
            ...
            DemandOrgYearReportListLineMaxValDetailResultData detail = new DemandOrgYearReportListLineMaxValDetailResultData();
            ...
            detail.setMonthStartDateList(new ArrayList<>());
            detail.setElementNumResultList(new ArrayList<>());
            detail.setElementNumResultListForAll(new ArrayList<>());
        ...
        for (CommonDisplayElementListResult displayElement : displayElementList) {
            ...
            currentOrgYearReport = null;
            monthOrgStartMap = null;
            if (ApiCodeValueConstants.ELEMENT_TYPE.LINE_ALL.getVal().equals(displayElement.getElementType())
                ...
                headerTitleList
                        .add(new DemandOrgYearReportListLineMaxValHeaderTitleResultData(displayElement.getElementName(),
                                ApiSimpleConstants.UNIT_DEMAND, displayElement.getElementType(),
                                displayElement.getGraphColor()));
            } else {
                headerTitleList.add(new DemandOrgYearReportListLineMaxValHeaderTitleResultData(
                        displayElement.getElementName(),
                        displayElement.getUnit(), displayElement.getElementType(), displayElement.getGraphColor()));
            }
            ...
            if (ApiCodeValueConstants.ELEMENT_TYPE.LINE_ALL.getVal().equals(displayElement.getElementType())
                ...
                if (currentOrgYearReport != null) {
                    linePowerData = getLinePowerDataFromLine(parameter.getOperationCorpId(), parameter.getBuildingId(),
                            ...
                            summaryRange.getMonthTo(), parameter.getSummaryKind(), parameter.getPrecision(),
                            parameter.getBelowAccuracyControl(), displayElement.getElementType(), currentOrgYearReport);
                } else {
                    linePowerData = getLinePowerDataFromLine(parameter.getOperationCorpId(), parameter.getBuildingId(),
                            parameter.getLineGroupId(), displayElement.getLineNo(),
                            summaryRange.getYearFrom(), summaryRange.getYearTo(), summaryRange.getMonthFrom(),
                            summaryRange.getMonthTo(), parameter.getSummaryKind(), parameter.getPrecision(),
                            parameter.getBelowAccuracyControl(), displayElement.getElementType(), currentYearReport);
                }
                ...
                linePowerData = getLinePowerDataFromPoint(parameter.getOperationCorpId(), parameter.getBuildingId(),
                        ...
                        displayElement.getPointNo(), summaryRange.getYearFrom(), summaryRange.getYearTo(),
                        summaryRange.getMonthFrom(), summaryRange.getMonthTo(), parameter.getSummaryKind(),
            ...
            for (DemandOrgYearReportListLineMaxValDetailResultData detail : detailList) {
                if (monthOrgStartMap != null) {
                    detail.getMonthStartDateList().add(DateUtility
                            .changeDateFormat(monthOrgStartMap.get(detail.getYm().replace("/", "")),
                                    DateUtility.DATE_FORMAT_YYYYMMDD_SLASH)
                            .concat(ApiSimpleConstants.FROM));
                } else {
                    detail.getMonthStartDateList().add(DateUtility
                            .changeDateFormat(monthStartMap.get(detail.getYm().replace("/", "")),
                                    DateUtility.DATE_FORMAT_YYYYMMDD_SLASH)
                            .concat(ApiSimpleConstants.FROM));
                }
            }
        ...
        //契約電力を取得する
        for (DemandOrgYearReportListLineMaxValDetailResultData detail : detailList) {
            if (buildingDemandList == null) {
                //企業集計の場合、建物デマンド情報を取得する
                BuildingDemandListDetailResultData buildingDemandParam = DemandEmsUtility
                        .getBuildingDemandListParam(parameter.getOperationCorpId(), parameter.getBuildingId());
                buildingDemandList = getResultList(buildingDemandListServiceDaoImpl, buildingDemandParam);
                if (buildingDemandList == null || buildingDemandList.size() != 1) {
            ...
            if (contractPowerMax == null) {
                contractPowerMax = detail.getContractPower();
            } else if (detail.getContractPower() != null
                    && detail.getContractPower().compareTo(contractPowerMax) >= 0) {
                contractPowerMax = detail.getContractPower();
            }
        }
        ...
        for (DemandOrgYearReportListLineMaxValDetailResultData detail : detailList) {
            ...
            if (ApiGenericTypeConstants.SUMMARY_UNIT.CORP.getVal().equals(parameter.getSummaryKind())) {
                ...
                if (filterCorpCalList != null && filterCorpCalList.size() == 1) {
                    monthStartDate = filterCorpCalList.get(0).getMonthStartDate();
                }
            } else if (ApiGenericTypeConstants.SUMMARY_UNIT.BUILDING.getVal().equals(parameter.getSummaryKind())) {
                ...
                if (filterBuildingCalList != null && filterBuildingCalList.size() == 1) {
                    monthStartDate = filterBuildingCalList.get(0).getMonthStartDate();
                }
            }

            if (monthStartDate != null) {
                CommonDemandMonthReportListResult monthReportParam = DemandEmsUtility.getMonthReportListParam(
                        parameter.getOperationCorpId(), parameter.getBuildingId(), monthStartDate, monthStartDate);
                ...
                if (monthReportList != null && monthReportList.size() == 1) {
                    detail.setTargetPower(monthReportList.get(0).getTargetKw());
                    if (targetPowerMax == null) {
                        targetPowerMax = detail.getTargetPower();
                    } else if (detail.getTargetPower() != null
                            && detail.getTargetPower().compareTo(targetPowerMax) >= 0) {
                        targetPowerMax = detail.getTargetPower();
                    }
                }
            }
        ...
        for (CommonDisplayElementListResult displayElement : displayElementList) {
            ...
            if (ApiCodeValueConstants.ELEMENT_TYPE.LINE_ALL.getVal().equals(displayElement.getElementType())
                    || ApiCodeValueConstants.ELEMENT_TYPE.LINE_DEMAND.getVal()
                            .equals(displayElement.getElementType())) {
                headerTitleListForAll
                        .add(new DemandOrgYearReportListLineMaxValHeaderTitleResultData(displayElement.getElementName(),
                                ApiSimpleConstants.UNIT_DEMAND, displayElement.getElementType(),
                                displayElement.getGraphColor()));
            } else {
                headerTitleListForAll.add(new DemandOrgYearReportListLineMaxValHeaderTitleResultData(
                        displayElement.getElementName(),
                        displayElement.getUnit(), displayElement.getElementType(), displayElement.getGraphColor()));
            }
            ...
            if (ApiCodeValueConstants.ELEMENT_TYPE.LINE_ALL.getVal().equals(displayElement.getElementType())
                ...
                linePowerData = getLinePowerDataFromPoint(parameter.getOperationCorpId(), parameter.getBuildingId(),
                        ...
                        parameter.getSummaryKind(), parameter.getPrecision(), parameter.getBelowAccuracyControl(),
                        currentYearReport);
            } else if (ApiCodeValueConstants.ELEMENT_TYPE.AMEDAS.getVal().equals(displayElement.getElementType())) {
                linePowerData = getLinePowerDataFromAmedas(parameter.getOperationCorpId(), parameter.getBuildingId(),
                        summaryRange.getYearFrom(),
            ...
            if (linePowerData == null) {
                //系統別データが取得できない場合
                elementNumMaxListForAll.add(null);
                for (DemandOrgYearReportListLineMaxValDetailResultData detail : detailList) {
                    detail.getElementNumResultListForAll().add(null);
                }
            } else {
                //系統別データが取得できた場合
                elementNumMaxListForAll.add(linePowerData.getSummary().getMaxNumUsed());
                for (DemandOrgYearReportListLineMaxValDetailResultData detail : detailList) {
                    boolean flg = false;
                    for (CommonLinePowerYearDataDetailResult detailResult : linePowerData.getDetail()) {
                        if (detailResult.getYearNo()
                                .concat(String.format(StringUtility.STRING_FORMAT_ZERO_PADDING_2,
                                        detailResult.getMonthNo().intValue()))
                                .equals(detail.getYm().replace(ApiSimpleConstants.SLASH, ""))) {
                            detail.getElementNumResultListForAll().add(detailResult.getResultNumMax());
                            flg = true;
                            break;
                        }
                    }
        ...
        header.setHeaderTitleElementResultListForAll(headerTitleListForAll);
        result.setHeader(header);
        result.setDetailList(detailList);
        summary.setElementNumMaxList(elementNumMaxList);
        summary.setElementNumMaxListForAll(elementNumMaxListForAll);
        summary.setContractPowerMax(contractPowerMax);
        summary.setTargetPowerMax(targetPowerMax);
     ...
     */
    private List<CommonDisplayElementListResult> getDisplayElementList(String corpId, Long buildingId,
            Long lineGroupId, String lineNo) {
        ...
        DemandBuildingSmPointListDetailResultData buildingSmPointParam = DemandEmsUtility
                .getBuildingSmPointListParam(corpId, buildingId, null, null);
        List<DemandBuildingSmPointListDetailResultData> buildingSmPointList = getResultList(
        ...
        //建物デマンド情報を取得する
        BuildingDemandListDetailResultData buildingDemandParam = DemandEmsUtility.getBuildingDemandListParam(corpId,
                buildingId);
        List<BuildingDemandListDetailResultData> buildingDemandList = getResultList(buildingDemandListServiceDaoImpl,
                buildingDemandParam);
        if (buildingDemandList != null && buildingDemandList.size() == 1
