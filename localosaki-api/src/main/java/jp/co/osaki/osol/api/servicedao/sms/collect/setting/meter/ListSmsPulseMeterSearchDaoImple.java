...
/**
 * メーター管理 パルスメーター情報一覧取得 ServiceDaoクラス
 * @author sagi_h
 *
 */
public class ListSmsPulseMeterSearchDaoImple implements BaseServiceDao<ListSmsMeterResultData> {
    ...
    public List<ListSmsMeterResultData> getResultList(ListSmsMeterResultData target, EntityManager em) {
    	boolean fromWeb = target.getDevName().contentEquals("fromWeb");
    	target.setDevName(null);
        ...
        // [共通] 装置情報TBL
        Join<MMeter, MDevPrm> joinDevPrm = root.join(MMeter_.MDevPrm, JoinType.INNER);
        // 建物装置中間テーブル
        Join<MDevPrm, MDevRelation> joinDevRelation = joinDevPrm.join(MDevPrm_.MDevRelations, JoinType.LEFT);
        // [MuDM2]コンセントレータ処理用TBL
        Join<MDevPrm, MConcentrator> joinConcentrator = joinDevPrm.join(MDevPrm_.MConcentrators, JoinType.LEFT);
        ...
        // MM.DEV_ID = MDP.DEV_ID and MDP.DEL_FLG = '0'
        joinDevPrm.on(
                builder.and(
                        builder.equal(
                                root.get(MMeter_.id).get(MMeterPK_.devId), joinDevPrm.get(MDevPrm_.devId)),
                        builder.equal(
                                joinDevPrm.get(MDevPrm_.delFlg), OsolConstants.FLG_OFF)));
        ...
        joinDevRelation.on(
                builder.and(
                        builder.equal(
                                ...
                                joinDevRelation.get(MDevRelation_.id).get(MDevRelationPK_.devId))));
        ...
        // MDP.DEV_ID = MC.DEV_ID and MM.CONCENT_ID = MC.CONCENT_ID
        joinConcentrator.on(
                builder.and(
                        builder.equal(
                                joinDevPrm.get(MDevPrm_.devId),
                                joinConcentrator.get(MConcentrator_.id).get(MConcentratorPK_.devId)),
                        builder.equal(
                                root.get(MMeter_.concentId),
                                joinConcentrator.get(MConcentrator_.id).get(MConcentratorPK_.concentId))));

        // 建物メーター関係との関連(装置ID、メーター管理番号、所属建物の建物ID除外)
        joinBuildDevMeterRelation.on(
                builder.and(
                        builder.equal(
                                root.get(MMeter_.id).get(MMeterPK_.meterMngId),
                                joinBuildDevMeterRelation
                                        .get(TBuildDevMeterRelation_.id).get(TBuildDevMeterRelationPK_.meterMngId)),
                        builder.equal(
                                root.get(MMeter_.id).get(MMeterPK_.devId),
                                joinBuildDevMeterRelation
                                        .get(TBuildDevMeterRelation_.id).get(TBuildDevMeterRelationPK_.devId)),
        ...
        // MM.DEV_ID = 'M388819Y' /* $2 */ and SUBSTR(MM.DEV_ID, 1, 2) <> 'MH' and SUBSTR(MM.METER_ID, 1, 1) = 'P'
        ...
        whereList.add(builder.like(root.get(MMeter_.meterId), METER_KIND.PULSE.getVal() + "%"));
        ...
        // ■SELECT句の整理
       	query = query.select(builder.construct(ListSmsMeterResultData.class,
                ...
                joinConcentrator.get(MConcentrator_.name),
                root.get(MMeter_.ifType),
                ...
                root.get(MMeter_.currentData),
                root.get(MMeter_.currentDataChg),
                root.get(MMeter_.pulseType),
                root.get(MMeter_.pulseTypeChg),
                root.get(MMeter_.pulseWeight),
                root.get(MMeter_.pulseWeightChg),
                ...
                root.get(MMeter_.alertPauseFlg),
                root.get(MMeter_.meterStaMemo))).distinct(true).where(builder.and(whereList.toArray(new Predicate[] {})));
        ...
        if (!fromWeb) {
    		ListSmsMeterResultData resultTemp = new ListSmsMeterResultData();
        	int size = resultList.size();
        	for (int i = 0; i < size; i++) {
        		resultTemp = resultList.get(i);
        		resultTemp.setDevId(target.getDevId());
        		resultList.set(i, resultTemp);
        	}
        }
    	return resultList;
    }
