...
import jp.co.osaki.osol.api.dao.smcontrol.MeasurePointUpdateDao;
import jp.co.osaki.osol.api.dao.smcontrol.SmCntrolDao;
import jp.co.osaki.osol.api.parameter.smcontrol.MeasurePointUpdateParameter;
import jp.co.osaki.osol.api.result.smcontrol.MeasurePointUpdateResult;
...
import jp.co.osaki.osol.mng.param.A200056Param;
 ...
 *
 */
@Named(value = SmControlConstants.MEASURE_POINT_UPDATE)
@RequestScoped
public class MeasurePointUpdateBean extends AbstractApiBean<MeasurePointUpdateResult, MeasurePointUpdateParameter>{

    @EJB
    private MeasurePointUpdateDao dao;
    ...
    @Override
    protected <T extends BaseParam> T initParam(MeasurePointUpdateParameter parameter) {
        A200056Param param = new Gson().fromJson(parameter.getResult(), A200056Param.class);

        param.setMeasurePointInfo(parameter.getMeasurePointInfo());
        param.setUpdateDBflg(parameter.isUpdateDBflg());

        // コマンド設定
        if (SmControlConstants.MEASURE_POINT_INFO_1.equals(param.getMeasurePointInfo())) {
            param.setCommand("X0" + SmControlConstants.MEASURE_POINT_INFO_1_CMD);
        } else if (SmControlConstants.MEASURE_POINT_INFO_2.equals(param.getMeasurePointInfo())) {
            param.setCommand("X0" + SmControlConstants.MEASURE_POINT_INFO_2_CMD);
        } else {
            // Not Process
        }
    ...
    protected boolean checkSmPrm(SmPrmResultData smPrm, BaseParam param) throws SmControlException {
        ...
        //listサイズチェック
        List<Map<String, String>> measurePointInfoList = ((A200056Param)param).getMeasurePointInfoList();

        if ((super.isEa(smPrm) && measurePointInfoList.size() != SmControlConstants.MEASURE_POINT_UPDATE_INFO_LIST_E_ALPHA)
                || (super.isEa2(smPrm) && measurePointInfoList.size() != SmControlConstants.MEASURE_POINT_UPDATE_INFO_LIST_E_ALPHA_2)) {
            StackTraceElement st = Thread.currentThread().getStackTrace()[1];
            super.loggingError(st, "measurePointInfoList.size()", String.valueOf(measurePointInfoList.size()));
            throw new SmControlException(OsolApiResultCode.API_ERROR_PARAMETER_VALID, "API_ERROR_PARAMETER_VALID");
        }

        String measurePointInfo = ((A200056Param)param).getMeasurePointInfo();
        if (!SmControlConstants.MEASURE_POINT_INFO_1.equals(measurePointInfo)
                && !SmControlConstants.MEASURE_POINT_INFO_2.equals(measurePointInfo)) {
            StackTraceElement st = Thread.currentThread().getStackTrace()[1];
            super.loggingError(st, "measurePointInfo", measurePointInfo);
            throw new SmControlException(OsolApiResultCode.API_ERROR_PARAMETER_VALID, "API_ERROR_PARAMETER_VALID");
        }

        // 機種固有パラメータチェック
        if (super.isEa2(smPrm)) {
            for (Map<String, String> info : measurePointInfoList) {
                String powerSaveMeasure = info.get("powerSaveMeasure");

                if ((powerSaveMeasure == null) || !(powerSaveMeasure.matches("[0-9]"))) {
                    StackTraceElement st = Thread.currentThread().getStackTrace()[1];
                    super.loggingError(st, "powerSaveMeasure", powerSaveMeasure);
                    throw new SmControlException(OsolApiResultCode.API_ERROR_PARAMETER_VALID, "API_ERROR_PARAMETER_VALID");
                }
            }
        }
    ...
    protected void callDao(FvpCtrlMngResponse<?> response) throws Exception {
        ...
        // param取得
        @SuppressWarnings("unchecked")
        FvpCtrlMngResponse<A200056Param> res = (FvpCtrlMngResponse<A200056Param>) response;
        A200056Param param = res.getParam();
