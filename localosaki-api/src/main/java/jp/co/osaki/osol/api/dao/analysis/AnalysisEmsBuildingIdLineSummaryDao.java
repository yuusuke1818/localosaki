...
/**
 *
 * 集計・分析 EMS実績 系統別サマリー取得(建物ID指定) Daoクラス
 *
 * @author y-maruta
 *
 */
@Stateless
public class AnalysisEmsBuildingIdLineSummaryDao extends OsolApiDao<AnalysisEmsBuildingIdLineSummaryParameter> {
    ...
    public AnalysisEmsBuildingIdLineSummaryDao() {
    ...
    @Override
    public AnalysisEmsBuildingIdLineSummaryResult query(AnalysisEmsBuildingIdLineSummaryParameter parameter)
            throws Exception {

        AnalysisEmsBuildingIdLineSummaryResult result = new AnalysisEmsBuildingIdLineSummaryResult();
        result.setLineSummaryList(new ArrayList<AnalysisEmsAllLineSummaryResultData>());
        ...
        targetBuildingParam.setBuildingId(parameter.getBuildingId());
        targetBuildingParam.setBelongTenantFlg(parameter.getBelongTenantFlg());
        ...
        List<AnalysisAllBuildingListResultData> targetBuildingresultList = getResultList(analysisBuildingIdListServiceDaoImpl,targetBuildingParam);
        ...
        for (AnalysisAllBuildingListResultData buildingSummaryResult : targetBuildingresultList) {
            ...
            for (AnalysisEmsAllLineSummaryResultData buildingSummary : buildingSummaryResultList) {
                ...
                for(AnalysisEmsAllLineSummaryResultData resultData : result.getLineSummaryList()) {
                    //全体サマリーデータに系統、時限が一致するデータがある場合は集計処理を行う。
                    if(buildingSummary.getLineNo().equals(resultData.getLineNo()) && buildingSummary.getJigenNo().equals(resultData.getJigenNo())) {
                        //合計
                        ...
                        if(resultData.getMax().compareTo(buildingSummary.getMax()) < 0 ) {
                            resultData.setSummary(buildingSummary.getMax());
                        }
                ...
                if(!existFlg) {
                    ...
                    result.getLineSummaryList().add(
                                new AnalysisEmsAllLineSummaryResultData(
        ...
        //平均値を算出する
        for (AnalysisEmsAllLineSummaryResultData buildingSummary : result.getLineSummaryList()) {
            //平均
