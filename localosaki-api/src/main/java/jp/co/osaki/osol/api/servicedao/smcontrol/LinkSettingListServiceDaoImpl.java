...
/**
 * 連動設定情報取得 ServiceDaoクラス
 * @author ya-ishida
 *
 */
public class LinkSettingListServiceDaoImpl implements BaseServiceDao<LinkSettingExtractResultData> {
    ...
    public List<LinkSettingExtractResultData> getResultList(LinkSettingExtractResultData target, EntityManager em) {
        ...
        //機器ID
        whereList.add(builder.equal(root.get(MSmAirSetting_.id).get(MSmAirSettingPK_.smId), smId));
        ...
        query = query.select(builder.construct(LinkSettingExtractResultData.class,
                ...
                root.get(MSmAirSetting_.version)))
