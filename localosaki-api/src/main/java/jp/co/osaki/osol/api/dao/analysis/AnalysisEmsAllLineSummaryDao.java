...
/**
 *
 * 集計・分析 EMS実績 系統別サマリー取得(全建物) Daoクラス
 *
 * @author y-maruta
 *
 */
@Stateless
public class AnalysisEmsAllLineSummaryDao extends OsolApiDao<AnalysisEmsAllLineSummaryParameter> {
    ...
    public AnalysisEmsAllLineSummaryDao() {
    ...
    @Override
    public AnalysisEmsAllLineSummaryResult query(AnalysisEmsAllLineSummaryParameter parameter) throws Exception {

        AnalysisEmsAllLineSummaryResult result = new AnalysisEmsAllLineSummaryResult(new ArrayList<>());
        ...
        List<AnalysisAllBuildingListResultData> targetBuildingresultList = getResultList(analysisAllBuildingListServiceDaoImpl,targetBuildingParam);
        ...
        for (AnalysisAllBuildingListResultData buildingSummaryResult : targetBuildingresultList) {
            ...
            for (AnalysisEmsAllLineSummaryResultData buildingSummary : buildingSummaryResultList) {
                ...
                //系統、時限のデータが存在しなかった場合、建物別サマリーデータのレコードを初期値として格納する(建物IDは不要なので削除する)
                if(!existFlg) {
