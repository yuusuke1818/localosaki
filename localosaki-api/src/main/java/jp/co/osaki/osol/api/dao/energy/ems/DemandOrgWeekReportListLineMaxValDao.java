...
import jp.co.osaki.osol.api.resultdata.energy.ems.DemandOrgWeekReportListLineMaxValDetailResultData;
import jp.co.osaki.osol.api.resultdata.energy.ems.DemandOrgWeekReportListLineMaxValHeaderResultData;
import jp.co.osaki.osol.api.resultdata.energy.ems.DemandOrgWeekReportListLineMaxValHeaderTitleResultData;
import jp.co.osaki.osol.api.resultdata.energy.ems.DemandOrgWeekReportListLineMaxValSummaryAreaResultData;
...
/**
 * エネルギー使用状況実績取得（個別・週報・系統別最大値） Daoクラス
 *
 * @author ya-ishida
 */
@Stateless
public class DemandOrgWeekReportListLineMaxValDao extends OsolApiDao<DemandOrgWeekReportListLineMaxValParameter> {
    ...
    public DemandOrgWeekReportListLineMaxValDao() {
     ...
     */
    @Override
    public DemandOrgWeekReportListLineMaxValResult query(DemandOrgWeekReportListLineMaxValParameter parameter)
            throws Exception {
        DemandOrgWeekReportListLineMaxValResult result = new DemandOrgWeekReportListLineMaxValResult();
        DemandOrgWeekReportListLineMaxValHeaderResultData header = new DemandOrgWeekReportListLineMaxValHeaderResultData();
        List<DemandOrgWeekReportListLineMaxValHeaderTitleResultData> headerTitleList = new ArrayList<>();
        List<DemandOrgWeekReportListLineMaxValDetailResultData> detailList = new ArrayList<>();
        DemandOrgWeekReportListLineMaxValSummaryAreaResultData summary = new DemandOrgWeekReportListLineMaxValSummaryAreaResultData();
        ...
        if (ApiGenericTypeConstants.SUMMARY_UNIT.CORP.getVal().equals(parameter.getSummaryKind())) {
            ...
            if (corpDemandList == null || corpDemandList.size() != 1) {
                return new DemandOrgWeekReportListLineMaxValResult();
            }
        ...
        do {
            ...
            DemandOrgWeekReportListLineMaxValDetailResultData detail = new DemandOrgWeekReportListLineMaxValDetailResultData();
        ...
        for (CommonDisplayElementListResult displayElement : displayElementList) {
            if (ApiCodeValueConstants.ELEMENT_TYPE.LINE_ALL.getVal().equals(displayElement.getElementType())
                ...
                headerTitleList
                        .add(new DemandOrgWeekReportListLineMaxValHeaderTitleResultData(displayElement.getElementName(),
                                ApiSimpleConstants.UNIT_DEMAND, displayElement.getElementType(),
                                displayElement.getGraphColor()));
            } else {
                headerTitleList.add(new DemandOrgWeekReportListLineMaxValHeaderTitleResultData(
            ...
            if (ApiCodeValueConstants.ELEMENT_TYPE.LINE_ALL.getVal().equals(displayElement.getElementType())
                ...
                linePowerData = getLinePowerDataFromLine(parameter.getOperationCorpId(), parameter.getBuildingId(),
                        ...
                        summaryRange.getWeekNoTo(), parameter.getSummaryKind(), parameter.getPrecision(),
            ...
            if (linePowerData == null) {
                ...
                for (DemandOrgWeekReportListLineMaxValDetailResultData detail : detailList) {
