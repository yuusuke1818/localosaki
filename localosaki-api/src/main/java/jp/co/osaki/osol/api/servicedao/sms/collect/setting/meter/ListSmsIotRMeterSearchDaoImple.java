...
/**
 * メーター管理 IoT-R連携用メーター情報一覧取得 ServiceDaoクラス
 *
 * @author y.nakamura
 */
public class ListSmsIotRMeterSearchDaoImple implements BaseServiceDao<ListSmsMeterResultData> {
    ...
    public List<ListSmsMeterResultData> getResultList(ListSmsMeterResultData target, EntityManager em) {
        ...
        // 条件2: 装置IDがTWから始まる
        whereList.add(builder.like(root.get(MMeter_.id).get(MMeterPK_.devId), DEVICE_KIND.IOTR.getVal() + "%"));
