...
import jp.co.osaki.sms.batch.dao.CheckDevCommandAlartDao;
...
/**
 * コマンド送信異常メール送信実行 TimerServiceクラス
 *
 * @author akr_iwamoto
 *
 */
...
public class CheckDevCommandAlartTimerService extends SmsBatchTimerService implements Serializable {
     ...
     * DAO
     */
    private CheckDevCommandAlartDao dao;
    ...
    protected void start() {
        ...
        // Daoのインスタンス生成
        this.dao = new CheckDevCommandAlartDao(this.entityManager);
    ...
    protected void execute() {
        ...
        for (DevCommandDeviceListResultSet tDevPrm : tDevPrmList) {
            try {
                ...
                batchLogger.info(String.format("コマンド送信異常メール送信実行開始("
                ...
                long mWaitCount = dao.getWaitCommandCount(devId);
                batchLogger.trace(String.format("Wait command count=%d",mWaitCount));

                long mSendingCount = dao.getSendingCommandCount(devId);
                batchLogger.trace(String.format("Sending command count=%d",mSendingCount));
                ...
                if (mWaitCount >= flgCount ||
                        mSendingCount >= flgCount) {

                    // 本文テンプレート作成
                    BaseVelocity mailTemplate = new BaseVelocity(SmsBatchConstants.MAIL_TEMPLATE_CHECK_DEVCOMMAND_ALART,
                    ...
                    mailTemplate.put("sendingCount", mSendingCount);
                    // 件名
                    ...
                    batchLogger.info(String.format("コマンド送信異常メール送信実行終了("
                            + " dev_id = %s devSta = %s devName = %s homeDirectory = %s revFlg = %s",
                            devId, devSta, devName, homeDirectory, revFlg));
                }
