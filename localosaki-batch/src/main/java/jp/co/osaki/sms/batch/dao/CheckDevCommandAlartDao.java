 ...
 *
 */
public class CheckDevCommandAlartDao extends SmsBatchDao {
     ...
     * @param entityManager
     */
    public CheckDevCommandAlartDao(EntityManager entityManager) {
        super(entityManager);
    }
    ...
    /**
     * コマンド送信の処置待ち処理フラグ件数を取得
     *
     * @param String devId
     * @return 件数
     */
    public Long getWaitCommandCount(String devId) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<Long> query = cb.createQuery(Long.class);
        ...
        // 条件2:devId
        conditionList.add(cb.equal(root.get(TCommand_.srvEnt), TCOMMAND_SRV_ENT.WAIT.getVal()));
     ...
     * @return 件数
     */
    public Long getSendingCommandCount(String devId) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<Long> query = cb.createQuery(Long.class);
        ...
        conditionList.add(cb.equal(root.get(TCommand_.id).get(TCommandPK_.devId), devId));
        // 条件2:devId
        conditionList.add(cb.equal(root.get(TCommand_.srvEnt), TCOMMAND_SRV_ENT.SENDING.getVal()));
    ...
    public MPauseMailResultSet getDevSendMailFlg(String devId) {
        ...
        query.select(builder.construct(MPauseMailResultSet.class, root.get(MPauseMail_.commandErr)))
                .where(builder.and(conditionList.toArray(new Predicate[] {}))).distinct(true);
