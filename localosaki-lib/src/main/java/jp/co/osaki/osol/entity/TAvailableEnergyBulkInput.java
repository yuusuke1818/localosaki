...
/**
 * The persistent class for the t_available_energy_bulk_input database table.
 * 
 */
@Entity
@Table(name = "t_available_energy_bulk_input")
@NamedQueries({
    @NamedQuery(name = "TAvailableEnergyBulkInput.findAll", query = "SELECT t FROM TAvailableEnergyBulkInput t"),
    @NamedQuery(name = "TAvailableEnergyBulkInput.energyInputYearSearch",
            query = "SELECT t FROM TAvailableEnergyBulkInput t "
            + "WHERE t.id.corpId =:corpId "
            + "AND t.delFlg =:delFlg "
            + "ORDER BY t.processStartDatetime DESC")})
public class TAvailableEnergyBulkInput implements Serializable {
	private static final long serialVersionUID = 1L;

	@EmbeddedId
	private TAvailableEnergyBulkInputPK id;
	...
	public TAvailableEnergyBulkInput() {
	}

	public TAvailableEnergyBulkInputPK getId() {
		return this.id;
	}

	public void setId(TAvailableEnergyBulkInputPK id) {
		this.id = id;
	}
