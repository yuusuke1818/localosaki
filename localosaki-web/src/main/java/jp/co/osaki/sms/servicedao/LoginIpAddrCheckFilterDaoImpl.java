 ...
 * @author take_suzuki
 */
public class LoginIpAddrCheckFilterDaoImpl implements BaseServiceDao<MLoginIpAddr> {

    /**
     * ログイン許可IPアドレスリスト取得
     *
     * @param target ログイン許可IPアドレスリストEntity
     * @param em エンティティーマネージャー
     * @return　ログイン許可IPアドレス情報リスト
     */
    @Override
    ...
    public void remove(MLoginIpAddr target, EntityManager em) {
        MLoginIpAddr a = em.find(MLoginIpAddr.class, target.getId());
        em.remove(a);
