...
/**
 * ロードサーベイ 指定期間内の各メーターの使用量・指針値取得 全装置対応
 * ServiceDaoクラス
 *
 */
public class ListSmsDataViewAllDevDaoImpl implements BaseServiceDao<ListSmsDataViewResultData> {
    ...
    private List<ListSmsDataViewResultData> selectDevMeterValueDataList(EntityManager em,
        ...
        for(String devId: devIdList) {
            ...
            if (target.isForwardDiract()) {
                // 正方向の場合
                query = createForwardDiractQuery(builder, devId, meterMngIdList, getDateList);
            } else {
                // 逆方向の場合
                query = createReverseDiractQuery(builder, devId, meterMngIdList, getDateList);
            }
    ...
    private CriteriaQuery<ListSmsDataViewResultData> createForwardDiractQuery(CriteriaBuilder builder, String devId,
            Collection<Long> meterMngIdList, Collection<String> getDateClct) {
        ...
        if (CollectionUtils.isNotEmpty(meterMngIdList)) {
            whereClct.addAll(CriteriaUtility.createInCollection(meterMngIdPath, meterMngIdList));
        }
