 ...
 * @author yoneda_y
 */
public class MDevPrmCollectListServiceDaoImpl implements BaseServiceDao<MDevPrm> {
    ...
    public List<MDevPrm> getResultList(MDevPrm target, EntityManager em) {
        ...
        //ハンディ端末(MHから始まる装置ID)
        whereList.add(builder.notLike(root.get(MDevPrm_.devId), "MH%"));
        //AieLink(OCから始まる装置ID)
        whereList.add(builder.notLike(root.get(MDevPrm_.devId), "OC%"));
